...
public class SmsTransactionProcessorTest extends AbstractDbTest {
    ...
    public void testFindingPlaceholderIndexes() throws Exception {
        final Placeholder[] vals = Placeholder.values();

        int[] indexes = smsProcessor.findPlaceholderIndexes("Pokupka. Karta *<:A:>. Summa <:P:> RUB. NOVYY PROEKT, MOSCOW. <:D:>. Dostupno <:B:> RUB. Tinkoff.ru");
        ...
        indexes = smsProcessor.findPlaceholderIndexes("ECMC<:A:> <:D:> покупка <:P:> TEREMOK METROPOLIS Баланс: <:B:>р");
        Assert.assertTrue(indexes[Placeholder.ACCOUNT.ordinal()] == 0);
        Assert.assertTrue(indexes[Placeholder.DATE.ordinal()] == 1);
        Assert.assertTrue(indexes[Placeholder.PRICE.ordinal()] == 2);
        Assert.assertTrue(indexes[Placeholder.BALANCE.ordinal()] == 3);

        indexes = smsProcessor.findPlaceholderIndexes("ECMC<:A:> <:D:> покупка <:P:> TEREMOK METROPOLIS Баланс:");
        Assert.assertTrue(indexes[Placeholder.ACCOUNT.ordinal()] == 0);
        Assert.assertTrue(indexes[Placeholder.DATE.ordinal()] == 1);
        Assert.assertTrue(indexes[Placeholder.PRICE.ordinal()] == 2);
        Assert.assertTrue(indexes[Placeholder.BALANCE.ordinal()] == -1);

        indexes = smsProcessor.findPlaceholderIndexes("ECMC<:A:> <:D:><::> <:P:> TEREMOK METROPOLIS Баланс:");
